...
const EVENT_DATE = new Date('August 3, 2019 10:00 GMT+9');
...
export class Countdown {
  ...
  init() {
    const digitsElements = Array.from(this.countdownContainer.querySelectorAll('.js-digit'));
  ...
  assignRandomColor(digit, leftDigit, rightDigit) {
    ...
    digit.element.querySelectorAll(`.number-${digit.currentNumber}`).forEach(el => {
  ...
  getTime() {
    ...
    const minutesLeft = Math.floor((timeLeft - daysLeft * 86400 - hoursLeft * 3600) / 60);
