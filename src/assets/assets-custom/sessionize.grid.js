...
function sessionize_showmodal(n, t, i) {
  ...
  return (
      ...
      if (this.readyState === 4 && this.status === 200)
        for (
          r.innerHTML = this.responseText,
            r.classList.remove('is-hidden'),
            t = document.getElementsByClassName('sz-modal__close-on-click'),
            n = 0;
          n < t.length;
          n++
        )
    ...
    u.open(
      'POST',
      'https://sessionize.com/api/v2/' + n + '/' + t + '?id=' + i,
      !0
    ),
    u.send(),
    !1
...
function sessionize_inittabs() {
  var i = document.getElementsByClassName('sz-tab-container'),
    t,
    n;
  if (i.length > 1) {
    for (n = 0; n < i.length; n++)
      i[n].classList.remove('sz-tab-container--active');
    for (
      t = document.getElementsByClassName('sz-tabs__item'),
        t.length > 0 && t[0].classList.add('sz-tabs__item--active'),
        sessionize_tabchanged(),
        n = 0;
      n < t.length;
      n++
    )
      t[n].children[0].onclick = sessionize_tabchange;
  }
}
function sessionize_tabchange(n) {
  for (
    var r, i = document.getElementsByClassName('sz-tabs__item'), t = 0;
    t < i.length;
    t++
  )
    i[t].classList.remove('sz-tabs__item--active');
  return (
    (r = n.target.parentElement),
    r.classList.add('sz-tabs__item--active'),
    sessionize_tabchanged(),
    !1
  );
}
function sessionize_tabchanged() {
  ...
  if (i.length > 0) {
    for (
      t = document.getElementsByClassName('sz-tab-container'), n = 0;
      n < t.length;
      n++
    )
      t[n].classList.remove('sz-tab-container--active');
    r = i[0].children[0].href.split('#')[1];
    document.getElementById(r).classList.add('sz-tab-container--active');
  }
}
function sessionize_onload() {
  if (
    (sessionize_inittabs(),
    window.location.hash && window.location.hash.lastIndexOf('#sz-', 0) === 0)
  ) {
...
window.onload = sessionize_onload;
